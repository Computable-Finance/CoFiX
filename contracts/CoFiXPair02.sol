...
contract CoFiXPair02 is ICoFiXPair02, CoFiXERC20 {
    ...
    function _queryOracle(address token, CoFiX_OP op, bytes memory data) internal returns (uint256, uint256, uint256, uint256, uint256) {
        return ICoFiXController04(ICoFiXFactory02(factory).getController()).queryOracle{value: msg.value}(token, uint8(op), data);
    }
    ...
    function _safeSendFeeForCoFiHolder(address _token0, uint256 _fee) internal {
        address feeReceiver = ICoFiXFactory02(factory).getFeeReceiver();
    ...
    function _safeSendFeeForLP(address _token0, address _token1, uint256 _fee) internal {
        address feeVault = ICoFiXFactory02(factory).getFeeVaultForLP(_token1);
