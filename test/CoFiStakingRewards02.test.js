...
const CoFiStakingRewards = artifacts.require("CoFiStakingRewards02.sol");
...
contract('CoFiStakingRewards', (accounts) => {
    ...
    before(async () => {
        ...
        CDAO = await CoFiXDAO.new(CoFi.address, StakingRewards.address, CFactory.address); 
        await CFactory.setDAO(CDAO.address);
    });
    ...
    it("should exit (withdraw & getReward) correctly", async () => {
        ...
        const ethBalanceBefore = await CDAO.totalETHRewards();
        ...
        const ethBalanceAfter = await CDAO.totalETHRewards();
        if (verbose) {
        ...
        expect(ethBalanceAfter.sub(ethBalanceBefore)).to.bignumber.equal(savingAmount);
    });
    
    // it("should withdrawAllSavingByGov correctly", async () => {
    //     const savingAmount = await StakingRewards.pendingSavingAmount();
    //     if (verbose) {
    //         console.log(`savingAmount: ${savingAmount}`);
    //     }
    ...
    //     const ethBalanceBefore = await CDAO.totalETHRewards();
    //     await StakingRewards.withdrawAllSavingByGov({ from: CoFi_User1});
    //     const newSavingAmount = await StakingRewards.pendingSavingAmount();
    //     const ethBalanceAfter = await CDAO.totalETHRewards();
