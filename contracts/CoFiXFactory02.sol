...
contract CoFiXFactory02 is ICoFiXFactory02 {
    ...
    function pairCodeHash() external pure returns (bytes32) {
        return keccak256(type(CoFiXPair02).creationCode);
    }
    ...
    function createPair(address token, uint256 initToken0Amount, uint256 initToken1Amount) external override onlyGovernance returns (address pair) {
        ...
        bytes memory bytecode = type(CoFiXPair02).creationCode;
        ...
        ICoFiXPair02(pair).initialize(WETH, token, _name, _symbol, initToken0Amount, initToken1Amount);

        ICoFiXController04(controller).addCaller(pair);
