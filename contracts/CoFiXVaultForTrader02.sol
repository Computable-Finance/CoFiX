...
contract CoFiXVaultForTrader02 is ICoFiXVaultForTrader02, ReentrancyGuard {
    ...
    function calcNeededETHAmountForAdjustment(address pair, uint256 ethAmount, uint256 erc20Amount) public override view returns (uint256) {
        /*
        ...
        */
        ...
        {
            (uint256 initToken0Amount, uint256 initToken1Amount) = ICoFiXPair02(pair).getInitialAssetRatio();
            (uint256 reserve0, uint256 reserve1) = ICoFiXPair02(pair).getReserves();
    ...
    */
    function actualMiningAmount(
        ...
        neededETHAmount = calcNeededETHAmountForAdjustment(pair, ethAmount, erc20Amount); // D_t
    ...
    function distributeReward(
        ...
        uint256 amount;
        {
            uint256 totalAccruedAmount;
            uint256 neededETHAmount;
            (amount, totalAccruedAmount, neededETHAmount) = actualMiningAmount(pair, ethAmount, erc20Amount);
    ...
    function clearPendingRewardOfCNode() external override nonReentrant {
        address vaultForCNode = ICoFiXFactory02(factory).getVaultForCNode();
    ...
    function clearPendingRewardOfLP(address pair) external override nonReentrant {
        address vaultForLP = ICoFiXFactory02(factory).getVaultForLP();
