...
import "./interface/ICoFiXRouter03.sol";
...
import "./interface/ICoFiXVaultForTrader02.sol";


// Router contract to interact with each CoFiXPair, no owner or governance
contract CoFiXRouter03 is ICoFiXRouter03 {
    ...
    function pairFor(address _factory, address token) internal view returns (address pair) {
        ...
        return ICoFiXFactory02(_factory).getPair(token);
    }
    ...
    ) external override payable ensure(deadline) returns (uint liquidity)
    {
        ...
        (liquidity, oracleFeeChange) = ICoFiXPair02(pair).mint{value: _oracleFee}(to);
    ...
    ) external override payable ensure(deadline) returns (uint liquidity)
    {
        ...
        uint256 oracleFeeChange;
        (liquidity, oracleFeeChange) = ICoFiXPair02(pair).mint{value: _oracleFee}(address(this));
        require(liquidity >= liquidityMin, "CRouter: less liquidity than expected");

        // find the staking rewards pool contract for the liquidity token (pair)
        address pool = ICoFiXVaultForLP(ICoFiXFactory02(factory).getVaultForLP()).stakingPoolForPair(pair);
        require(pool != address(0), "CRouter: invalid staking pool");
        // approve to staking pool
        ICoFiXPair02(pair).approve(pool, liquidity);
        ICoFiXStakingRewards(pool).stakeForOther(to, liquidity);
        ICoFiXPair02(pair).approve(pool, 0); // ensure
        // refund oracle fee to msg.sender, if any
        if (oracleFeeChange > 0) TransferHelper.safeTransferETH(msg.sender, oracleFeeChange);
    }
    ...
    ) external override payable ensure(deadline) returns (uint amountToken, uint amountETH) 
    {
        ...
        address pair = pairFor(factory, token);
        ICoFiXPair02(pair).transferFrom(msg.sender, pair, liquidity);

        uint oracleFeeChange; 
        (amountToken, amountETH, oracleFeeChange) = ICoFiXPair02(pair).burn{value: msg.value}(to, address(this));
    ...
    ) external override payable ensure(deadline) returns (uint _amountIn, uint _amountOut)
    {
        ...
        (_amountIn, _amountOut, oracleFeeChange, tradeInfo) = ICoFiXPair02(pair).swapWithExact{
        ...
        address vaultForTrader = ICoFiXFactory02(factory).getVaultForTrader();
        if (tradeInfo[0] > 0 && rewardTo != address(0) && vaultForTrader != address(0)) {
            ICoFiXVaultForTrader02(vaultForTrader).distributeReward(pair, tradeInfo[1], tradeInfo[2], rewardTo);
        }
    ...
    function swapExactTokensForTokens(
        ...
        uint oracleFeeChange;
        uint256[5] memory tradeInfo;
        (_amountIn, _amountOut, oracleFeeChange, tradeInfo) = ICoFiXPair02(pairs[0]).swapWithExact{value: msg.value}(WETH, address(this));

        // distribute trading rewards - CoFi!
        address vaultForTrader = ICoFiXFactory02(factory).getVaultForTrader();
        if (tradeInfo[0] > 0 && rewardTo != address(0) && vaultForTrader != address(0)) {
            ICoFiXVaultForTrader02(vaultForTrader).distributeReward(pairs[0], tradeInfo[1], tradeInfo[2], rewardTo);
        }

        // swapExactETHForTokens
        pairs[1] = pairFor(factory, tokenOut);
        assert(IWETH(WETH).transfer(pairs[1], _amountOut)); // swap with all amountOut in last swap
        (, _amountOut, oracleFeeChange, tradeInfo) = ICoFiXPair02(pairs[1]).swapWithExact{value: oracleFeeChange}(tokenOut, to);
        require(_amountOut >= amountOutMin, "CRouter: got less than expected");

        // distribute trading rewards - CoFi!
        if (tradeInfo[0] > 0 && rewardTo != address(0) && vaultForTrader != address(0)) {
            ICoFiXVaultForTrader02(vaultForTrader).distributeReward(pairs[1], tradeInfo[1], tradeInfo[2], rewardTo);
        }
    ...
    ) external override payable ensure(deadline) returns (uint _amountIn, uint _amountOut)
    {
        ...
        uint oracleFeeChange; 
        uint256[5] memory tradeInfo;
        (_amountIn, _amountOut, oracleFeeChange, tradeInfo) = ICoFiXPair02(pair).swapWithExact{value: msg.value}(WETH, address(this));
        require(_amountOut >= amountOutMin, "CRouter: got less than expected");
    ...
    function _swapOnCoFiX(uint i, address[] memory path, DEX_TYPE[] memory dexes, uint[] memory amounts, address _to, address rewardTo) internal {
            ...
            {
                uint256[5] memory tradeInfo;
                (,amounts[i+1],,tradeInfo) = ICoFiXPair02(pair).swapWithExact{value: NEST_ORACLE_FEE}(path[i + 1], to);
