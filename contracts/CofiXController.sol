...
contract CofiXController {
    ...
    mapping(address => KInfo) public getKInfo;
    ...
    function queryOracle(address token, address payback) external payable returns (uint256 _k, uint256, uint256, uint256) {
...

        {
            ...
            K = ABDKMath64x64.add(
                            ...
                            ABDKMath64x64.add(
                                ...
                                ABDKMath64x64.mul(BETA_TWO, ABDKMath64x64.fromUInt(_op.T))
                            )
                        );
            
            emit newK(token, K, _sigma, _op.T, _op.ethAmount, _op.erc20Amount, _op.blockNum);
        }
...

        {
            ...
            getKInfo[token].k = _k;
            getKInfo[token].updatedAt = block.timestamp;
