...
import "./interface/ICoFiXDAO.sol";

// Stake CoFi to earn ETH
contract CoFiStakingRewards02 is ICoFiStakingRewards, ReentrancyGuard {
    ...
    function getReward() public override nonReentrant updateReward(msg.sender) {
        ...
        if (pendingSavingAmount > 0) {
            address _to = ICoFiXFactory02(factory).getDAO();
            require(_to != address(0), "CoFiStaking: dao not set");
            uint256 _amount = pendingSavingAmount;
            uint256 bal = IWETH(rewardsToken).balanceOf(address(this));
            if (_amount > bal) {
                _amount = bal;
            }

            pendingSavingAmount = 0;
            IWETH(rewardsToken).withdraw(_amount);
            ICoFiXDAO(_to).addETHReward{value: _amount}();
